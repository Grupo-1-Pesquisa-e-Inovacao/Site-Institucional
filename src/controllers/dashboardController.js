...
function findAllEstados(req, res) {
  dashboardModel.findAllEstados()
    .then(estados => {
      res.json(estados);
    })
    .catch(erro => {
      console.error("Erro ao buscar todos os estados:", erro);
      res.status(500).json({ erro: "Erro ao buscar todos os estados" });
    });
}
